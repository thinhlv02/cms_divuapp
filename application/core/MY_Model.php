...
//    /conga1411 comment
    ...
    function get_list($input = array(), $field = '')
    {
        if ($field) {
            $this->db->select($field);
        }
...
//    function get_list_where( $input = array())
//    {
//
//    }
    ...
    protected function get_list_set_input($input = array())
    {
        ...
        if((isset($input['where_in'])) && $input['where_in']){
            $name = $input['where_in'][0];
            $values = $input['where_in'][1];
            $this->db->where_in($name, $values);
        }
//        where_not_in

        if((isset($input['where_not_in'])) && $input['where_not_in']){
            $name = $input['where_not_in'][0];
            $values = $input['where_not_in'][1];
            $this->db->where_not_in($name, $values);
        }
//        or_where_not_in
        if((isset($input['or_where_not_in'])) && $input['or_where_not_in']){
            $name = $input['or_where_not_in'][0];
            $values = $input['or_where_not_in'][1];
            $this->db->or_where_not_in($name, $values);
        }

        if((isset($input['or_where'])) && $input['or_where']){
            $name = $input['or_where'][0];
            $values = $input['or_where'][1];
            $this->db->or_where($name, $values);
        }
