                ...
                <div class="form-group">
                    ...
                    <div class="col-md-2 col-sm-2 col-xs-12">
                        <input type="text" id="txtFrom" name="date1" required
                               ...
                               elseif (isset($_GET['date1'])) echo $_GET['date1'] ?>"
                ...
                <table id="datatable-product" class="table table-striped table-bordered bulk_action">
                    ...
                    <?php if (isset($res) && count($res) > 0) foreach ($res as $key => $value): ?>
                        ...
                        <tr id="<?php echo $value->id; ?>"
                            ...
                            <td class=""><?php echo $value->service_package_name ?></td>
                            <td><?php echo $value->des ?></td>
                            <td><?php echo $value->name_status.'('. $value->status.')' ?></td>
...
//                                           echo 'id teach: ' . $row .'---'. $value->status. '<br>';
                                        ...
                                        if (!empty($itemMon)) { ?>
                                            <div id="<?php echo $row; ?>"
                                                ...
                                                <!--                                                <a href="-->
                                                <?php //echo base_url('admin/admin_mission/edit/' . $row . '/' . $value->id) ?><!--"><i-->
                                                <!--                                                            class="fa fa-pencil-square-o" aria-hidden="true"></i></a>-->
                                                <a <?php if (in_array($value->status, array(2,3,4,5,6,7))) echo 'class="disabled"' ?>
                                                        onclick="confirm_del_event(<?php echo $row ?>, <?php echo $value->id ?>, '<?php echo $date1 ?>', '<?php echo $date2 ?>', '<?php echo $itemMon->fullname ?>')"
                                                        class="btn btn-danger btn-xs"><i class="fa fa-trash-o fa-lg"
                                                                                         aria-hidden="true"></i><?php echo $value->name_status == 5 ?>
                                                </a>
                                            </div>
                                            <hr>
                                        <?php } ?>
                                ...
                                <div class="btn_them_1">
                                    <a <?php if ($value->status == 5) echo 'class="disabled"' ?>
                                            href="<?php echo base_url('admin/admin_mission/add/' . $value->id . '/' . $value->district_id . '/' . $value->province_id . '/' . $date1 . '/' . $date2) ?>"><i
...
<script type="text/javascript">
    function confirm_del_event(admin_id, id, date1, date2,fullname) {
        ...
        if (r == true) {
            console.log(admin_id + '--'+id+'--'+date1+'--'+date2);
            window.location.href = "<?php echo admin_url('admin_mission/del/')?>" + admin_id + "/" + id + '/' + date1 + '/' + date2;
        }
    }
</script>