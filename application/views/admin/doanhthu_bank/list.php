                ...
                <div class="table-responsive">
                    ...
                    <table id="datatable-product" class="table table-striped table-bordered bulk_action">
                        ...
                        <?php $total = 0; if (isset($res) && count($res) > 0) foreach ($res as $key => $value): ?>
                            ...
                            if ($value->transaction_status == '00') {
                                $transaction_status = 'Đã thanh toán';
                            }
                            if ($value->transaction_status == '01') {
                                $transaction_status = 'Đã thanh toán';
                            }
                            if ($value->transaction_status == '02') {
                                $transaction_status = 'Chưa thanh toán';
                            }

//                            $total = $value->pay_cards + $value->bank + $value->ktv;
                        $total += $value->tien_nap;
                            ?>
                            ...
                            <tr id="<?php ?>" class="<?php if (isset($_GET['asset_id']) && $_GET['asset_id'] == $value->id) echo 'bg_thanhly'; ?>">
                                <td><?php echo $i ?></td>
                                <td class="text-nowrap"><?php echo $value->account_id ?></td>
                                <td class="text-nowrap"><?php echo $value->type_account ?></td>
                                <td class="text-nowrap"><?php echo $value->fullname ?></td>
                                <td class="text-nowrap"><?php echo number_format($value->tien_nap) ?></td>
                                <td class="text-nowrap"><?php echo $value->bank_code ?></td>
                                <td class="text-nowrap"><?php echo $transaction_status ?></td>
