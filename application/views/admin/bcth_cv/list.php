...
<div class="page-title">
    <div class="title_left">
        <h3>Báo cáo tổng hợp công việc</h3>
    </div>
            ...
            <div class="x_content">
                ...
                <table id="datatable-product" class="table table-striped table-bordered bulk_action">
                    ...
                    <tr>
                        ...
                        <th class="text-nowrap">Họ và tên NV</th>
...
//                                $new_day = convert_name_day(date('D', strtotime($v))). '<br />';
                    ...
                    if (!empty($list_emp2) && count($list_emp2) != 0)
                        foreach ($list_emp2 as $key => $value): ?>
                            <?php
//                            $work = $this->technician_jobs_model->bcc_chitiet_tq($date1, $admin_id);
                            $count++; ?>
                            <tr>
...
//                                    foreach ($work as $k3 => $v3){ ?>
                                    ...
                                    <td class="text-nowrap">
                                        ...
                                        foreach ($work as $k3 => $v3) {
                                            $report_name = '';
                                            if (strpos($v3->name, 'Bảo dưỡng') !== false) {
                                                $report = $this->admin_mission_report_model->get_list(array('where' => array('admin_mission_id' => $v3->id)));
                                                if ($report) {
                                                    $report_name = explode('***', $report[0]->content);
                                                    $report_name = $report_name[1];
                                                }
                                            } else {
                                                $report = $this->admin_emergency_report_model->get_list(array('where' => array('admin_emergency_id' => $v3->id)));
                                                if ($report) {
                                            ...
                                            ?>
                                            <?php
//                                            echo (date('Y-m-d',strtotime($v2))). '<br/>';

                                            if ($v3->time_start_go != '') echo date('H:i:s', strtotime($v3->time_start_go)) . ' : ' . $report_name . '<br/>' ?>
                                        <?php }
