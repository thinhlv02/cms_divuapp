...
//                                'color' => array('rgb' => '6665ff')
    ...
    function add()
    {
        ...
        if ($this->input->post('btnAddemployee')) {
            ...
            $qdct = $this->input->post('qdct');
            $qdct = new DateTime($qdct);
            $qdct = $qdct->getTimestamp();
//            $displayName = $this->input->post('displayName');
            ...
            $ngay_bd = new DateTime($ngay_bd);
            $ngay_bd = $ngay_bd->getTimestamp();
            $phone = $this->input->post('phone');
            $hotline = $this->input->post('hotline');
            $ohientai = $this->input->post('ohientai');
            $address = $this->input->post('address');
            $hdtv = $this->input->post('hdtv');
            $email = $this->input->post('email');
            $email_cty = $this->input->post('email_cty');
            $hdld = $this->input->post('hdld');
            $hdld = new DateTime($hdld);
            $hdld = $hdld->getTimestamp();
            $qdnv = $this->input->post('qdnv');
            $position = $this->input->post('position');
            $noisinh = $this->input->post('noisinh');
            $department = $this->input->post('department');
            $noi_lamviec = $this->input->post('noi_lamviec');
...
//                'displayName' => $displayName,
            ...
            if ($this->upload->do_upload('img_news2')) {
                ...
                $dataSubmit['img'] = $file_data['file_name'];
            } else {
...
//                'displayName' => $displayName,
            ...
            if ($this->upload->do_upload('img_news2')) {
                $file_data = $this->upload->data();
                $dataSubmit['img'] = $file_data['file_name'];
                unlink('./public/images/employee/' . $employee->img);
            } else {
                $this->session->set_flashdata('message', $this->upload->display_errors('', ''));
//                return;
            }
...
//            }
        ...
        $this->data['temp'] = 'admin/employee/details';
        $this->load->view('admin/layout', $this->data);
    }
